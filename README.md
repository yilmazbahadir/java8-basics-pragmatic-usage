...
| int[] array = new int[] {<br/>     1, 2, 3, 4, 5, 6, 7, 8, 8, 8, 9, 10 <br/>}; <br/>int sum = 0;<br/> for (int j = 0; j < array.length; j++) {<br/> for (int k = j + 1; k < array.length; k++) { <br/>if (k != j && array[k] == array[j])       { <br/>            sum = sum + array[k]; <br/>           System.out.println(                "Duplicate found: "              + array[k]              + " "              + "Sum of the duplicate value is " + sum);       <br/> }   <br/> }}| the other part |
